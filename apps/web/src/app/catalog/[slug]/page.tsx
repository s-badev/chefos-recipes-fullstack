...
export default async function RecipeDetailsPage({ params }: RecipeDetailsPageProps) {
  ...
  if (!recipe) {
    return (
      <section className="mx-auto max-w-3xl rounded-[2rem] border border-dashed border-brand-200 bg-white/84 p-10 text-center shadow-sm">
        ...
        <Link
          className="mt-6 inline-flex rounded-full bg-brand-600 px-6 py-3 text-sm font-black text-white shadow-sm shadow-brand-900/20 transition hover:bg-brand-700"
          href="/catalog"
        >
  ...
  return (
    <article className="page-shell space-y-10">
      <Link className="inline-flex text-sm font-black text-brand-700 hover:text-brand-900" href="/catalog">
        Обратно към каталога
      </Link>
        ...
        <div className="editorial-card rounded-[1.8rem] p-6">
          ...
          <ul className="mt-5 grid gap-3 sm:grid-cols-2">
            {recipe.ingredients.map((ingredient) => (
              <li className="flex gap-3 text-[15px] leading-7 text-stone-700" key={ingredient}>
      ...
      <section className="editorial-card rounded-[1.8rem] p-6">
        ...
        <ol className="mt-6 grid gap-4">
          {recipe.steps.map((step, index) => (
            <li className="flex gap-4 text-[15px] leading-7 text-stone-700" key={step}>
