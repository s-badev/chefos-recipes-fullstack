...
export async function POST(request: Request) {
  ...
  if (!email || !password) {
    return jsonError(request, "Email and password are required.", 400);
  }
  ...
  try {
    ...
    return jsonResponse(request, { token, user });
  } catch (error) {
    return jsonError(request, error instanceof Error ? error.message : "Login failed.", 500);
  }
}
